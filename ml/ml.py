...
def train_model():
    try:
        ...
        model_dir = os.path.dirname(config.DURATION_MODEL_PATH)
        if model_dir and not os.path.exists(model_dir):
            os.makedirs(model_dir)

        with open(config.DURATION_MODEL_PATH, 'wb') as f:
            pickle.dump(model, f)
        ...
        return jsonify({
            ...
            'count': len(records),
            'model_path': config.DURATION_MODEL_PATH
        })
...
def predict_duration():
    try:
        if not os.path.exists(config.DURATION_MODEL_PATH):
            return jsonify({
                'success': False, 
                'message': f'Model bulunamadı: {config.DURATION_MODEL_PATH}. Önce modeli eğitmeniz gerekiyor.'
            })
        ...
        with open(config.DURATION_MODEL_PATH, 'rb') as f:
            model = pickle.load(f)
